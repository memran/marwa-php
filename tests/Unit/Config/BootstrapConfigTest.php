...
final class BootstrapConfigTest extends TestCase
{
    ...
    {
        $basePath = $GLOBALS['marwa_app']->basePath();

        $_ENV['APP_CONFIG_CACHE'] = $basePath . '/storage/cache/config.php';
        $_SERVER['APP_CONFIG_CACHE'] = $basePath . '/storage/cache/config.php';
        putenv('APP_CONFIG_CACHE=' . $basePath . '/storage/cache/config.php');

        $_ENV['APP_ROUTE_CACHE'] = $basePath . '/storage/cache/routes.php';
        $_SERVER['APP_ROUTE_CACHE'] = $basePath . '/storage/cache/routes.php';
        putenv('APP_ROUTE_CACHE=' . $basePath . '/storage/cache/routes.php');

        $_ENV['APP_MODULE_CACHE'] = $basePath . '/storage/cache/modules.php';
        $_SERVER['APP_MODULE_CACHE'] = $basePath . '/storage/cache/modules.php';
        putenv('APP_MODULE_CACHE=' . $basePath . '/storage/cache/modules.php');

        $config = BootstrapConfig::defaults($GLOBALS['marwa_app']);

        self::assertSame($basePath . '/storage/cache/config.php', $config['configCache']);
        self::assertSame($basePath . '/storage/cache/routes.php', $config['routeCache']);
        self::assertSame($basePath . '/storage/cache/modules.php', $config['moduleCache']);
    }
